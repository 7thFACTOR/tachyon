...
Mutex s_lockFbxUsage;

#ifdef IOS_REF
#undef IOS_REF
#define IOS_REF (*(m_pFbxMan->GetIOSettings()))
#endif
...
{
	m_pFbxMan = FbxManager::Create();
	// create an IOSettings object
	FbxIOSettings *ios = FbxIOSettings::Create(m_pFbxMan, IOSROOT);
	m_pFbxMan->SetIOSettings(ios);
...
#endif
	m_pFbxMan->LoadPluginsDirectory(lPath.Buffer(), lExtension.Buffer());

	// Create the entity that will hold the scene.
	m_pFbxScene = FbxScene::Create(m_pFbxMan, "");

	m_bExportMaterials = true;
	m_bExportModels = true;
	m_bExportMeshes = true;
	m_bSplitMeshRootsIntoModels = false;
}
...
bool FbxFileImporter::loadFbxScene(FbxManager* pSdkManager, FbxDocument* pScene, const char* pFilename)
{
	...
	strcpy(lPassword, m_assetPassword.c_str());
	...
	E_LOG_VERBOSE("FBX version number for this FBX SDK is %d.%d.%d", lSDKMajor, lSDKMinor, lSDKRevision);

	if (lImporter->IsFBX())
	{
		E_LOG_VERBOSE("FBX version number for file %s is %d.%d.%d", 
			pFilename, lFileMajor, lFileMinor, lFileRevision);

		// From this point, it is possible to access animation stack information without
		// the expense of loading the entire file.

		E_LOG_VERBOSE("Animation Stack Information");

		lAnimStackCount = lImporter->GetAnimStackCount();

		E_LOG_VERBOSE("    Number of Animation Stacks: %d", lAnimStackCount);
		E_LOG_VERBOSE("    Current Animation Stack: \"%s\"", lImporter->GetActiveAnimStackName().Buffer());
		E_LOG_VERBOSE("");
		...
		for (i = 0; i < lAnimStackCount; i++)
		{
			...
			E_LOG_VERBOSE("    Animation Stack %d", i);
			E_LOG_VERBOSE("         Name: \"%s\"", lTakeInfo->mName.Buffer());
			E_LOG_VERBOSE("         Description: \"%s\"", lTakeInfo->mDescription.Buffer());

			// Change the value of the import name if the animation stack should be imported
			// under a different name.
			E_LOG_VERBOSE("         Import Name: \"%s\"", lTakeInfo->mImportName.Buffer());

			// Set the value of the import state to false if the animation stack should be not
			// be imported.
			E_LOG_VERBOSE("         Import State: %s", lTakeInfo->mSelect ? "true" : "false");
			E_LOG_VERBOSE("");
		}
...

{
	...
	if (!loadFbxScene(m_pFbxMan, m_pFbxScene, pSrcFilename))
		return false;
