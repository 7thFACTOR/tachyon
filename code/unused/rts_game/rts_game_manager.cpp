...
N_REGISTER_PLUGIN(RtsGamePlugin, "A sample rts game");
N_REGISTER_DERIVED_CLASS(RtsGame, GameManager);

int numInstancesPerSide = 10;
Vec3* velos;

void RtsGame::onInitialize()
{
	files().addDataFolder("../data");
	files().addDataFolder("../engine");

	if (engineHost() == eEngineHost_GameLauncher)
	{
		loadSection("worlds/test.nsection", false);
	}

	m_pFont = nullptr;
	//m_cameraEntityId = StandardEntitiesFactory::composeCamera(this->createEmptyEntity());
	m_controls.load("../config/rts_game.input");
...
void RtsGame::onExitGameState()
{
}

void RtsGame::onUpdate()
{
	if (&editor())
	{
		editor().update();
...
m.setIdentity();
	...
	render().mainContext().clear(
		...
		Color(2.0f/255.0f, 120.0f/255.0f, 10.0f/255.0f, 0));
...
//	render().mainContext().setViewMatrix(m_pCameraEntity->component<Camera>()->viewMatrix());
	...
	render().mainContext().clear(
		eClearBufferFlag_Color|eClearBufferFlag_Depth,
		Color(42.0f/255.0f, 120.0f/255.0f, 250.0f/255.0f, 0));
...
void RtsGame::onInput()
{
	...
	//resources().load<Material>("materials/default.nmaterial");
