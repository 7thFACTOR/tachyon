...
const TCHAR* kHorusWndClass = "HorusWndClass";
LRESULT CALLBACK horusWndProc(HWND, UINT, WPARAM, LPARAM);
bool s_wndClassWasRegistered = false;
int iconId = 1;//TODO, expose to user
static std::map<HWND, WindowsNativeWindow*> s_nativeWindows;
bool s_mainWindowSet = false;
std::map<int, KeyCode> mapVKtoKeyCode;

KeyCode translateWinKeyToKeyCode(int wParam)
{
	return mapVKtoKeyCode[wParam];
}

u32 unicodeCharFromVk(int wParam)
{
	WCHAR keyBuff[32] = { 0 };
	int scanCode = MapVirtualKey(wParam, MAPVK_VK_TO_VSC);
	int res = ToUnicode(wParam, scanCode, NULL, (LPWSTR)keyBuff, 32, 0);
...
struct WindowsNativeWindow : INativeWindow
{
	HWND m_hwnd = 0;
	std::string m_internalName;
	NativeWindowListener* m_listener = nullptr;
	bool m_isMainWindow = false;
	HGLRC m_context = 0;

	~WindowsNativeWindow()
	{
		auto iter = s_nativeWindows.find(m_hwnd);
